...
SDL_AppResult LLWindowSDL::handleEvent(const SDL_Event& event)
{
    switch(event.type)
    {
        ...
        {
            // Keep keycode for unicode handling and grabby-keys checks
            ...
            // Use scancode for LL keyboard translation (fits U16)
            gKeyboard->handleKeyDown(static_cast<U16>(event.key.scancode), mKeyModifiers);

            if (mKeyVirtualKey == SDLK_RETURN || !mLanguageTextInputAllowed)
                mCallbacks->handleUnicodeChar(static_cast<llwchar>(mKeyVirtualKey), gKeyboard->currentMask(false));
        ...
        case SDL_EVENT_KEY_UP:
        {
            ...
            gKeyboard->handleKeyUp(static_cast<U16>(event.key.scancode), mKeyModifiers);
            break;
        }
