...
// System gl.h on some Linux distributions defines GL_VERSION_1_2/1_3 but
// omits the matching PFNGL* typedefs that llglfunctiondecls.h relies on.
// Undefining the version macros before including glext.h forces the Khronos
// header to provide the missing typedefs while re-establishing the macros.
...
// Some libGL implementations pull in a stub glext.h from the system that has
// already defined its include guard but still lacks the function pointer
// typedefs we need. Undefine those guards so we can force-include the Khronos
// version shipped with our build.
#ifdef __glext_h_
#undef __glext_h_
#endif
#ifdef __gl_glext_h_
...
// Ensure GLX-related typedefs (Display, XID, Bool, etc.) are defined before
// glh_extensions/glxext headers pull them in. The Khronos GL/glxext.h expects
// Xlib headers to be visible first, otherwise builds on some distros fail with
// errors like: 'XID does not name a type' or 'Display was not declared'.
#if defined(__linux__) || defined(__unix__) || defined(__FreeBSD__)
# include <X11/Xlib.h>
# include <X11/Xutil.h>
// Ensure GLX types (GLXContext, GLXDrawable, GLXPixmap, etc.) are defined
// before any inclusion of GL/glxext.h by third-party headers.
# include <GL/glx.h>
#endif

// The __APPLE__ kludge is to make glh_extensions.h not symbol-clash horribly
// Additionally, prefix glh_genext declared symbols to avoid collisions with
// prototypes provided by system GL headers on Mesa-based builds.
# define GLH_EXT_PREFIX _
# define __APPLE__
# include "GL/glh_extensions.h"
# undef __APPLE__
# undef GLH_EXT_PREFIX
